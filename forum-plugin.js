window.RochePlugin.register({
  ...
  apps: [{
    ...
    async mount(container, roche) {
      const defaultWorldView = `围绕着游戏/动漫角色本人发帖（崩铁、原神、鸣潮、绝区零、王者、咒术回战、夏目友人帐、银魂、鬼灭之刃、坂本日常、电锯人、排球少年、蓝色监狱、死亡笔记等）。
$禁止OOC，严格遵循官方人设。
$角色全洁，从身到心，只喜欢user一人。
...
②严禁来自耽美/腐向小说、漫画的角色出现（包括但不限于龙族、夜幕之下、斩神、诡秘之主、盗墓笔记等原著小说及其耽改版角色）。
③只允许游戏/非腐向动漫的角色发帖和评论。
...
<div class="roche-plugin-forum ${settings.themeStyle !== 'line' ? 'theme-' + settings.themeStyle : ''}" id="main-container">
    ...
    <div class="forum-content">
      ...
      <div class="form-section">
        ...
        <div class="collapsible-body" data-body="api">
          ...
          <label>读取之前帖子条数:<input type="number" id="set-prev-post-count" value="${settings.prevPostReadCount ?? 5}" min="0" max="30"></label>
  ...
  <!-- 加载遮罩：spinner 内容由 CSS ::before 控制 -->
  <div class="loading-mask" id="loading-mask">
    ...
    <div style="font-weight:bold;color:var(--primary-color);font-size:16px;margin-top:10px;" id="loading-text">正在捕捉时空交汇的电波...</div>
      ...
      const renderFav = () => {
        ...
        favorites.forEach(f => {
          ...
          d.querySelector('.post-stats').onclick = async (e) => {
            ...
            renderFav(); roche.ui.toast("已取消收藏");
          };
      ...
      const setReplyTarget = (authorName, commentId) => {
        ...
        replyingHint.classList.add('show'); commentInput.focus();
      };
      ...
      const openDetail = (id) => {
        ...
        let html = `<div style="display:flex;align-items:center;margin-bottom:8px;"><div class="bw-avatar" style="${avStyle}"></div><div class="skeleton-author">@${p.author}</div></div><div style="white-space:pre-wrap;">${renderAt(p.content)}</div><div class="skeleton-stats">——♡${likes} ★${stars} ＞${comments.length}</div><div style="color:var(--primary-color);margin:12px 0 8px;">|<br>|</div>`;
      ...
      const sendComment = async (text, parentId, parentAuthorName) => {
        ...
        clearReplyTarget(); openDetail(p.id);
        ...
        const floorScopeHint = floorRootAuthor
          ? `⚠️ 用户是在 @${floorRootAuthor} 的评论下回复的。请判断 @${floorRootAuthor} 属于哪个作品世界观，只生成同一世界观的角色续评，严禁其他世界观角色。`
        ...
        try {
          const sysPrompt = `你是乙女向论坛模拟器。${ETOGE_RULE}\n${USER_GENDER_RULE}\n${atUserRule()}\n世界观：${settings.worldView}`;
...
${floorScopeHint}
当前心情：${currentMood()}。
要求：生成2~4条角色评论/回复。严格遵循官方人设，禁止OOC，禁止图片描述。@用户时必须用 @${userProfile.forumName}。提到user时用"她"。
输出纯JSON数组（必须完整合法，不能截断）：[{"author":"角色名","content":"回复内容"}]`;
        ...
        if (p.author !== userProfile.forumName && Math.random() < 0.55) {
          try {
            ...
            const sys2 = `你是乙女向论坛模拟器。${ETOGE_RULE}\n${USER_GENDER_RULE}\n${atUserRule()}`;
            const user2 = `模拟帖主"@${p.author}"决定是否回复评论区。严格遵循官方人设，禁止OOC，当前心情：${currentMood()}。
帖主帖子：${p.content}
评论区：\n${existing}
user是女孩子，第三人称用"她"。@用户时必须用 @${userProfile.forumName}。
- 决定回复则输出：[{"author":"${p.author}","content":"@回复对象 内容","replyTo":"被回复的评论者名字"}]
- 决定不回复则输出：[]
输出纯完整JSON，不要其他文字。`;
      ...
      $('#detail-comment-summon').onclick = async () => {
        ...
        try {
          ...
          const sysP = `你是乙女向论坛模拟器。${ETOGE_RULE}\n${USER_GENDER_RULE}\n${atUserRule()}\n世界观：${settings.worldView}`;
...
${scopeHint}
当前心情：${currentMood()}。严格遵循官方人设，禁止OOC，禁止图片描述。@用户时必须用 @${userProfile.forumName}。
输出纯JSON数组（必须完整合法）：[{"author":"角色名","content":"回复内容"}]`;
      ...
      $('#btn-test-api').onclick = async () => {
        ...
        try {
          ...
          else { const txt = await res.text(); resultEl.innerHTML = `<div class="api-status fail">❌ 连接失败，状态码：${res.status}<br><small>${txt.slice(0,200)}</small></div>`; }
...
- @用户时必须用 @${userProfile.forumName}，禁止用旅行者/开拓者等世界观称呼。
- 提到user时第三人称用"她"。
- emoji根据人设判断，不强制。`;
...
输出JSON数组：[{"author":"角色名","content":"正文","likes":数字,"stars":数字,"comments":[{"author":"评论人","content":"内容","parentIndex":null或父评论0起始index}]}]`;
...
输出JSON数组：[{"author":"角色名","content":"正文","likes":数字,"stars":数字,"comments":[{"author":"评论人","content":"内容","parentIndex":null或父评论0起始index}]}]`;
        ...
        } catch (err) { console.error(err); showError(err.message || String(err), err.raw || ''); }
        finally { loadingMask.style.display = 'none'; }
      };
