window.RochePlugin.register({
  ...
  apps: [{
    ...
    async mount(container, roche) {
      ...
      // ========== 1. 初始化数据 ==========
      const defaultWorldView = `围绕着游戏/动漫发帖人就是角色本人发帖 如: 崩铁、原神、鸣潮、王者、排球少年、蓝色监狱、咒术回战、夏目友人帐、银魂、鬼灭之刃等不是作者设定为耽美的高热度游戏动漫角色
...
$评论禁止出现开拓者本人/旅行者本人！user就是开拓者/旅行者！`;
      ...
      const systemWorldViewFallback = `这是一个乙女向论坛体，发帖角色都是游戏、动漫、小说（小说禁耽美小说）人物角色，发帖主题随机，请根据角色本身的人设、性格、日常来自由决定帖子主题（可以参考：日常生活、小烦恼、工作、小幸运、对user暗戳戳的思念/表白/吃醋，但不限于此，可以发任何符合角色人设的内容，要有活人感）。`;

      const ETOGE_RULE = `【重要：本论坛是乙女向/女性向。
①严禁BL/腐向/耽美内容，严禁男男恋爱描写。
②严禁来自耽美/腐向小说、漫画的角色出现。
③只允许游戏/非腐向动漫的角色发帖和评论。
④所有角色只对用户（女孩子）产生好感、暧昧、爱慕情感。违反任何一条视为无效输出，必须重新生成。】`;

      const USER_GENDER_RULE = `【user性别规则：user是女孩子，第三人称一律用"她"，严禁用"他"称呼user。】`;
      ...
      let settings = (await roche.storage.get("forum_settings")) || {
        ...
        apiModel: "gpt-4o", useRocheAI: true,
        memoryReadCount: 5, prevPostReadCount: 5, selectedWorldbooks: []
      };
...
:root {
  ...
  --bg-color:#fff; --card-bg:#fff; --border-radius:0;
...
.theme-water {
  --primary-color:#a3b8cc; --bg-color:#f2f7fb; --border-radius:16px;
  --border-style:1px solid rgba(163,184,204,0.4);
  --box-shadow:0 4px 15px rgba(163,184,204,0.15); --avatar-color:#fff;
}
.theme-food {
  --primary-color:#fcaebf; --bg-color:#fdf5f7; --border-radius:24px;
  --border-style:2px dashed #fcaebf;
  --box-shadow:0 6px 20px rgba(252,174,191,0.2); --avatar-color:#fff;
}
.theme-green {
  --primary-color:#5aaa7a; --bg-color:#f0faf4; --card-bg:#ffffff;
  --border-radius:18px; --border-style:1.5px solid #a8dbbe;
  --box-shadow:0 3px 12px rgba(90,170,122,0.13); --avatar-color:#fff;
  --font-text:'PingFang SC','Microsoft YaHei',sans-serif;
}
.theme-green .forum-header { background:linear-gradient(135deg,#e3f5eb,#f0faf4); }
.theme-green .forum-bottom-bar { background:#e8f7ee; }
...
.forum-header {
  ...
  padding:16px; background:var(--bg-color); border-bottom:var(--border-style); z-index:5;
}
.forum-header button {
...
.bw-avatar {
  width:36px; height:36px; border-radius:50%; background:var(--avatar-color);
  border:1px solid var(--primary-color); flex-shrink:0; margin-right:12px;
...
.forum-bottom-bar {
  display:flex; justify-content:space-around; padding:18px 10px;
  background:var(--bg-color); border-top:var(--border-style);
...
.forum-bottom-bar button {
  ...
  cursor:pointer; flex:1; color:var(--primary-color); font-family:var(--font-text); padding:4px 0;
}
.forum-bottom-bar button.active { text-decoration:underline; text-underline-offset:4px; }
.form-section {
  border:var(--border-style); padding:16px; border-radius:var(--border-radius);
  margin-bottom:16px; background:var(--card-bg);
}
.form-section h4 {
  margin:0 0 12px; border-bottom:1px solid var(--primary-color); padding-bottom:6px;
...
.btn-primary {
  ...
  padding:12px 16px; font-weight:bold; cursor:pointer; width:100%; margin-top:10px;
}
.btn-danger {
...
.btn-secondary {
  background:var(--bg-color); color:var(--primary-color); border:var(--border-style);
  border-radius:8px; padding:8px 12px; font-weight:bold; cursor:pointer; font-size:14px;
}
.detail-view {
...
.comment-divider { color:var(--primary-color); font-size:16px; margin:8px 0; user-select:none; letter-spacing:2px; }
.comment-item {
  display:flex; align-items:flex-start; gap:8px; padding:6px 0;
  cursor:pointer; border-radius:4px; transition:background 0.1s;
}
.comment-item:hover { background:rgba(0,0,0,0.03); }
.comment-item.is-reply { padding-left:16px; border-left:2px solid var(--primary-color); margin-left:8px; }
.comment-body { flex:1; }
...
.comment-input-area {
  display:flex; padding:12px; background:var(--card-bg);
  border-top:var(--border-style); gap:6px;
}
.comment-input-area input {
  flex:1; margin:0; border-radius:20px; padding:10px 16px; border:1px solid var(--primary-color);
}
.comment-input-area button {
  background:var(--primary-color); color:#fff; border:none; border-radius:20px;
  padding:0 12px; font-weight:bold; cursor:pointer; font-size:13px; white-space:nowrap;
}
.btn-refresh-comment {
...
.my-tabs button {
  flex:1; padding:12px; background:none; border:none; font-family:var(--font-text);
  font-weight:bold; color:var(--primary-color); cursor:pointer; font-size:14px;
}
...
.theme-green .decor-green { display:block; }
...
<div class="roche-plugin-forum ${settings.themeStyle !== 'line' ? 'theme-' + settings.themeStyle : ''}" id="main-container">
  ...
  <div class="decor decor-green" style="top:12%;right:8%">🌿</div>
  <div class="decor decor-green" style="bottom:25%;left:6%">🍃</div>
  ...
  <div class="forum-header">
    ...
    <button id="nav-refresh-modal">↻</button>
  </div>
    ...
    <div class="forum-content">
      <div class="form-section">
        ...
        <div class="collapsible-body open" data-body="theme">
          <select id="theme-style">
            ...
            <option value="green" ${settings.themeStyle==='green'?'selected':''}>清新浅绿：薄荷风 🍡</option>
          </select>
      ...
      <div class="form-section">
        ...
        <div class="collapsible-body" data-body="api">
          ...
          <p style="font-size:12px;color:#888;margin:4px 0 12px;">勾选：先用 Roche 内置 AI，失败才 fallback 自定义 API。</p>
          ...
          <label>记忆总结条数:<input type="number" id="set-memory-count" value="${settings.memoryReadCount}" min="0" max="30"></label>
          <label>生成参考的过往帖子数:<input type="number" id="set-prev-post-count" value="${settings.prevPostReadCount ?? 5}" min="0" max="30"></label>
  ...
  <div class="detail-view" id="view-post-detail">
    ...
    <div class="comment-input-area">
      ...
      <button id="detail-comment-summon" class="btn-refresh-comment">刷新评论</button>
    </div>
  </div>

  <!-- 刷新生成前弹窗 -->
  <div class="page-view" id="modal-refresh" style="position:absolute;top:0;left:0;width:100%;height:100%;z-index:15;display:none;flex-direction:column;background:var(--bg-color);">
    <div class="forum-header">
      <button id="refresh-cancel" style="font-size:16px;">取消</button>
      <div style="font-weight:bold;">刷新设定</div>
      <button id="refresh-submit" style="font-size:16px;font-weight:bold;">开始生成</button>
    </div>
    <div style="padding:16px;">
      <label style="font-weight:bold;">自定义本次世界观/剧情（选填）：</label>
      <p style="font-size:12px;color:#888;margin:4px 0;">如果不填，将默认读取设置中的世界观配置。</p>
      <textarea id="refresh-custom-worldview" placeholder="例如：今天学校举办了运动会..." style="height:150px;"></textarea>
    </div>
  </div>
  ...
  <div class="loading-mask" id="loading-mask">
    ...
    <div style="font-weight:bold;color:var(--primary-color);font-size:16px;margin-top:10px;" id="loading-text">正在捕捉时空交汇的电波...</div>
      ...
      const userAvatarStyle = () => userProfile.avatarUrl ? `background-image:url(${userProfile.avatarUrl});background-color:transparent;` : `background-color:var(--primary-color);`;
      const atUserRule = () => `【@用户规则：当角色评论或回复中需要@用户时，必须使用用户的论坛代号 @${userProfile.forumName}，严禁使用"旅行者""开拓者""星""莹"等世界观内称呼代替论坛代号。】`;
      
      const getFinalWorldView = (customWv) => {
        let wv = customWv || settings.worldView.trim();
        if (!wv) wv = systemWorldViewFallback;
        return wv;
      };
      ...
      const showError = (msg, raw) => {
        ...
        overlay.innerHTML = `<div class="error-box"><h3>⚠️ 生成失败</h3><pre class="error-pre">${(msg||'未知错误')+(raw?'\n\n返回内容：\n'+raw:'')}</pre><div style="text-align:right;"><button class="btn-primary" style="width:auto;padding:8px 20px;" id="err-close">关闭</button></div></div>`;
      ...
      const safeParseJSON = (raw) => {
        if (!raw || typeof raw !== 'string') throw new Error('AI返回内容为空');
        const start = raw.indexOf('[');
        if (start === -1) throw Object.assign(new Error('返回内容中未找到 JSON 数组'), { raw: raw.slice(0, 500) });
        const end = raw.lastIndexOf(']');
        if (end !== -1) { try { return JSON.parse(raw.substring(start, end + 1)); } catch(e) {} }
        const partial = raw.substring(start);
        const lastClose = partial.lastIndexOf('}');
        if (lastClose === -1) throw Object.assign(new Error('JSON截断且无法恢复'), { raw: raw.slice(0, 500) });
        try { return JSON.parse(partial.substring(0, lastClose + 1) + ']'); }
        catch(e2) { throw Object.assign(new Error('JSON截断恢复失败：' + e2.message), { raw: raw.slice(0, 500) }); }
      };
      ...
      const callAI = async (systemPrompt, userPrompt) => {
        ...
        if (useRoche) {
          try { const r = await roche.ai.chat({ messages, temperature: 0.85 }); if (r && r.text) return r.text.trim(); }
          catch (e) { console.warn("Roche 内置 AI 失败，fallback...", e); }
        }
        if (settings.apiUrl && settings.apiKey) {
          const res = await fetch(settings.apiUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${settings.apiKey}` },
            body: JSON.stringify({ model: settings.apiModel || 'gpt-4o', messages, temperature: 0.85 })
          });
          if (!res.ok) { const txt = await res.text(); throw Object.assign(new Error(`HTTP ${res.status}`), { raw: txt }); }
      ...
      let currentMode = 'feed', boardSub = 'cross';
      ...
      $$('[data-toggle]').forEach(h => { h.onclick = () => $(`[data-body="${h.dataset.toggle}"]`).classList.toggle('open'); });
      ...
      const loadWorldbooks = async () => {
        ...
        try {
          ...
          wbEl.innerHTML = categories.map(cat => `<div style="margin-bottom:6px;"><label style="margin:0;display:flex;align-items:center;gap:8px;"><input type="checkbox" class="wb-check" value="${cat.id}" ${(settings.selectedWorldbooks||[]).includes(cat.id)?'checked':''} style="width:auto;margin:0;"><span>${cat.name || cat.id}</span></label></div>`).join('');
        } catch (e) { wbEl.innerHTML = '<div style="color:#888;">加载失败</div>'; }
      };
      ...
      const getWorldbookText = async () => {
        ...
        for (const catId of checked) {
          try {
            ...
            if (entries && entries.length > 0) text += entries.map(e => `${(e.keys||[]).join(',')}: ${e.content}`).join('\n') + '\n';
          } catch(e) {}
        }
        return text;
      };
      ...
      const renderList = (el, data) => {
        ...
        data.forEach(post => {
          ...
          d.innerHTML = `<div class="post-header"><div class="bw-avatar" style="${avStyle}"></div><div class="post-author">@${post.author}</div></div><div class="post-text collapsed">${renderAt(post.content)}</div><div class="post-stats">——♡${likes} ★${stars} ＞${cc}</div>`;
      ...
      const renderMine = () => {
        ...
        mine.forEach(p => {
          ...
          d.innerHTML = `<div class="post-header"><div class="bw-avatar" style="${userAvatarStyle()}"></div><div class="post-author">@${p.author}</div></div><div class="post-text collapsed">${renderAt(p.content)}</div><div class="post-stats">${p.comments?.length||0} 条回复</div>`;
      ...
      const renderFav = () => {
        ...
        favorites.forEach(f => {
          const d = document.createElement('div'); d.className = 'forum-post';
          d.innerHTML = `<div class="post-header"><div class="bw-avatar"></div><div class="post-author">@${f.author}</div></div><div class="post-text collapsed">${renderAt(f.content)}</div><div class="post-stats" style="color:#ff4d4f;cursor:pointer;">✕ 取消收藏</div>`;
      ...
      // ========== 详情页 - 扁平评论 ==========
      const buildFlatCommentsHTML = (comments, postAuthor) => {
        if (!comments || comments.length === 0) return '';
        let html = '';
        comments.forEach((c, idx) => {
          if (idx > 0) html += `<div class="comment-divider">· · ·</div>`;
          const isUser = c.author === userProfile.forumName;
          const isAuthor = c.author === postAuthor;
          const avStyle = isUser ? userAvatarStyle() : '';
          const badge = isAuthor ? `<span class="comment-author-badge">楼主</span>` : '';
          const isReply = !!c.parentId;
          html += `<div class="comment-item${isReply ? ' is-reply' : ''}" data-cid="${c.id}" data-cauthor="${c.author}"><div class="bw-avatar small" style="${avStyle}"></div><div class="comment-body"><span class="skeleton-author">@${c.author}</span>${badge}<br><span>${renderAt(c.content)}</span></div></div>`;
        });
        return html;
      };
      ...
      let curPostId = null, replyParentId = null, replyParentAuthor = null;

      const setReplyTarget = (authorName, commentId) => {
        replyParentId = commentId; replyParentAuthor = authorName;
        replyingHintText.innerText = `正在回复 @${authorName}`;
        replyingHint.classList.add('show');
        commentInput.focus();
      };
      const clearReplyTarget = () => {
        replyParentId = null; replyParentAuthor = null;
      ...
      const openDetail = (id) => {
        ...
        let html = `
          ...
          <div style="color:var(--primary-color);margin:12px 0 8px;">|<br>|</div>`;

        const flatHTML = buildFlatCommentsHTML(comments, p.author);
        html += flatHTML || '<div style="color:#888;">暂无评论，在下方输入或点击「刷新评论」</div>';
        detailC.innerHTML = html;

        detailC.querySelectorAll('.comment-item').forEach(row => {
          row.onclick = (e) => { e.stopPropagation(); setReplyTarget(row.dataset.cauthor, row.dataset.cid); };
        });
      ...
      // ========== 发送评论 ==========
      const sendComment = async (text, parentId, parentAuthorName) => {
        const loc = locatePost(curPostId); if (!loc) return;
        const p = loc.arr.find(x => x.id === curPostId);
        p.comments = p.comments || [];
        const fullText = parentAuthorName ? `@${parentAuthorName} ${text}` : text;
        const newComment = { id: crypto.randomUUID(), author: userProfile.forumName, content: fullText, parentId: parentId || null };
        ...
        if (parentId) {
          const findRoot = (cid) => { const c = p.comments.find(x => x.id === cid); if (!c || !c.parentId) return c; return findRoot(c.parentId); };
        ...
        const floorScopeHint = floorRootAuthor
          ? `⚠️ 用户是在 @${floorRootAuthor} 的评论下回复的。请判断 @${floorRootAuthor} 属于哪个作品世界观，只生成同一世界观的角色续评，严禁其他世界观角色出现在这条回复链。`
          : (loc.kind === 'feed' ? '请只生成同世界观角色回复。' : loc.kind === 'cross' ? '这是【跨界大乱炖】板块，允许不同世界观角色。' : '这是【if 线】板块，评论只能是帖主同一世界观角色。');

        loadingText.innerText = "正在呼唤角色回复...";
        loadingMask.style.display = 'flex';
        try {
          const finalWv = getFinalWorldView();
          const memArr = loc.kind === 'feed' ? memoryFeed : (loc.kind === 'if' ? memoryIf : memoryCross);
          const memText = memArr.slice(0, settings.memoryReadCount).map(m => `[${m.time}] ${m.summary}`).join('\n') || '（无）';
          
          const sysPrompt = `你是乙女向论坛模拟器。\n${ETOGE_RULE}\n${USER_GENDER_RULE}\n${atUserRule()}\n世界观：${finalWv}\n论坛记忆：\n${memText}`;
          const userPrompt = `用户（论坛代号 @${userProfile.forumName}，真名/爱称[${userProfile.name}]，女孩子，第三人称用"她"）刚在帖子下评论了。
原帖作者：@${p.author}
原帖内容：${p.content}
用户评论：${fullText}
${floorScopeHint}
当前心情：${currentMood()}。
要求：生成1~2条角色评论/回复。严格遵循官方人设，禁止OOC，禁止图片描述。emoji按人设判断。@用户时必须用 @${userProfile.forumName}，只@一次即可。提到user时用"她"。
输出纯JSON数组（必须完整合法，不能截断）：[{"author":"角色名","content":"回复内容"}]`;
          ...
          await roche.storage.set(loc.key, loc.arr);
          openDetail(p.id);
        } catch (e) { console.warn("角色回复失败", e); roche.ui.toast("角色暂时没回复"); }
        ...
        if (p.author !== userProfile.forumName && Math.random() < 0.55) {
          try {
            ...
            const sys2 = `你是乙女向论坛模拟器。\n${ETOGE_RULE}\n${USER_GENDER_RULE}\n${atUserRule()}`;
            const user2 = `模拟帖主"@${p.author}"决定是否回复评论区。严格遵循官方人设，禁止OOC，当前心情：${currentMood()}。
帖主帖子：${p.content}
评论区：\n${existing}
user是女孩子，第三人称用"她"。@用户时必须用 @${userProfile.forumName}（只@一次即可）。
- 决定回复则输出：[{"author":"${p.author}","content":"@回复对象名 回复内容","replyTo":"被回复的评论者名字"}]
- 决定不回复则输出：[]
输出纯完整JSON，不要其他文字。`;
            const raw2 = await callAI(sys2, user2);
            const arr2 = safeParseJSON(raw2);
            if (arr2.length > 0) {
              arr2.forEach(r => {
                let replyParId = null;
                if (r.replyTo) { const target = (p.comments||[]).slice().reverse().find(c => c.author === r.replyTo); if (target) replyParId = target.id; }
      ...
      $('#detail-comment-send').onclick = async () => {
        ...
        const pid = replyParentId, pAuthor = replyParentAuthor;
        commentInput.value = '';
        await sendComment(t, pid, pAuthor);
      };
      ...
      $('#detail-comment-summon').onclick = async () => {
        ...
        try {
          ...
          const existing = (p.comments||[]).map(c => `@${c.author}: ${c.content}`).join('\n') || '（无）';
          const scopeHint = loc.kind === 'feed' ? '只允许同世界观角色出现。' : loc.kind === 'cross' ? '【跨界大乱炖】板块，允许跨界角色。' : '【if 线】板块，评论必须来自帖主同一世界观。';
          const sysP = `你是乙女向论坛模拟器。\n${ETOGE_RULE}\n${USER_GENDER_RULE}\n${atUserRule()}\n世界观：${finalWv}\n论坛记忆：\n${memText}`;
          const userP = `请为下列帖子生成2~4条新的角色评论。
【帖子作者】@${p.author}
【帖子内容】${p.content}
【已有评论】\n${existing}
【用户论坛代号】@${userProfile.forumName}，真名/爱称[${userProfile.name}]，女孩子，提到user用"她"
${scopeHint}
当前心情：${currentMood()}。严格遵循官方人设，禁止OOC，禁止图片描述。@用户时必须用 @${userProfile.forumName}（只@一次）。
输出纯JSON数组（必须完整合法）：[{"author":"角色名","content":"回复内容"}]`;
          const raw = await callAI(sysP, userP);
          const replies = safeParseJSON(raw);
          replies.forEach(r => (p.comments = p.comments||[]).push({ id: crypto.randomUUID(), author: r.author, content: r.content, parentId: null }));
          await roche.storage.set(loc.key, loc.arr);
          openDetail(p.id);
          roche.ui.toast("评论已刷新");
        } catch (e) { showError(e.message || String(e), e.raw || ''); }
        finally { loadingMask.style.display = 'none'; }
      };
      ...
      $('#btn-fetch-models').onclick = async function() {
        ...
        try {
          ...
          wrap.innerHTML = `<label>选择模型:<select id="set-api-model">${models.map(m => `<option value="${m}" ${m===current?'selected':''}>${m}</option>`).join('')}</select></label><button class="btn-secondary" id="btn-fetch-models" style="width:100%;margin-top:4px;">🔄 重新拉取</button>`;
      ...
      // ========== 刷新生成弹窗逻辑 ==========
      $('#nav-refresh-modal').onclick = () => {
        if (currentMode !== 'feed' && currentMode !== 'crossover') switchView('feed', 'FORUM');
        $('#refresh-custom-worldview').value = '';
        $('#modal-refresh').style.display = 'flex';
      };
      $('#refresh-cancel').onclick = () => $('#modal-refresh').style.display = 'none';
      $('#refresh-submit').onclick = async () => {
        $('#modal-refresh').style.display = 'none';
        let refreshMode = currentMode, refreshSub = boardSub;
        const customWv = $('#refresh-custom-worldview').value.trim();
        const finalWv = getFinalWorldView(customWv);
        ...
        try {
          ...
          const prevPostsText = prevPosts.length ? prevPosts.map(p => {
            const cmtSummary = (p.comments || []).slice(0, 4).map(c => ` └@${c.author}: ${c.content.slice(0, 40)}`).join('\n');
            return `@${p.author}（帖子）: ${p.content.slice(0, 80)}\n${cmtSummary}`;
          }).join('\n\n') : '（无）';

          const existingAuthors = [...new Set(recentSrc.map(p => p.author))];
          const authorBlacklist = existingAuthors.length ? `⚠️ 以下角色最近已发帖，本次刷新帖主禁止重复（评论区可出现）：${existingAuthors.join('、')}` : '';
          const wbText = await getWorldbookText();

          const sysPrompt = `你是乙女向论坛模拟器，只输出合法JSON数组，不要输出任何其他文字、标点、代码块标记。
${ETOGE_RULE}
${USER_GENDER_RULE}
${atUserRule()}
【本次世界观设定】：
...
【世界书补充】：
${wbText || '（无）'}
【用户情报】：论坛代号 @${userProfile.forumName}，真名/爱称[${userProfile.name}]，年龄${userProfile.age}，外貌：${userProfile.appearance}。user是女孩子，第三人称用"她"。
【论坛记忆】：
${memText}
...
${authorBlacklist}
【发帖铁律】：
- 本次每篇帖子必须来自不同角色，严禁同一角色发多篇。
- 必须来自不同作品世界观，不允许同一作品连续发帖。
- 严格遵循官方人设，禁止OOC，禁止图片描述。
- @用户时必须用 @${userProfile.forumName}，禁止用旅行者/开拓者等称呼代替代号，只@一次即可。
- 提到user时第三人称用"她"。
- emoji根据人设判断，不强制。`;

          let userPrompt;
          if (refreshMode === 'feed') {
            userPrompt = `生成 ${settings.postCount} 篇帖子，每篇 ${settings.commentCount} 条评论。
发帖主题随机，请根据角色本身的人设、性格、日常来自由决定帖子主题（可以参考：日常生活、小烦恼、工作、小幸运、对user暗戳戳的思念/表白/吃醋，但不限于此，要有活人感）。
评论区每个顶层楼层只能有同世界观角色回复。
输出纯JSON数组（必须完整合法，不能截断）：[{"author":"角色名","content":"正文","likes":数字,"stars":数字,"comments":[{"author":"评论人","content":"内容","parentIndex":null或父评论0起始index}]}]`;
          } else if (refreshSub === 'cross') {
            userPrompt = `这是【跨界大乱炖】板块，允许不同世界观角色跨界串门。
围绕以下三种反应生成帖子（三选一或混合）：
① 各世界观角色发现 user 在他们那边也有"马甲"的震惊反应；
② 各世界观里暗恋 user 的角色聚在一起时的反应；
③ 某个世界观的角色发帖描述 user 在他们那边干过的具体事情。
生成 ${settings.postCount} 篇，每篇 ${settings.commentCount} 条评论。
输出纯JSON数组（必须完整合法，不能截断）：[{"author":"角色名","content":"正文","likes":数字,"stars":数字,"comments":[{"author":"评论人","content":"内容","parentIndex":null或父评论0起始index}]}]`;
          } else {
            userPrompt = `这是【if 线】板块——平行时空里帖主已和 user 在一起了。帖主秀恩爱，评论区其他角色暗恋 user 所以破防酸柠檬。
...
输出纯JSON数组（必须完整合法，不能截断）：[{"author":"角色名","content":"正文","likes":数字,"stars":数字,"comments":[{"author":"评论人","content":"内容","parentIndex":null或父评论0起始index}]}]`;
          ...
          const newPosts = arr.map(it => {
            ...
            const comments = withIds.map((c, i) => ({
              ...
              parentId: (c.parentIndex != null && c.parentIndex >= 0 && c.parentIndex < i) ? withIds[c.parentIndex].id : null
            }));
            return { id: crypto.randomUUID(), author: it.author || "未知", content: it.content || "", likes: it.likes ?? Math.floor(Math.random() * 500), stars: it.stars ?? Math.floor(Math.random() * 200), comments };
          });
